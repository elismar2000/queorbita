...
, em que
	...
	compile instructions: 
		  gcc queorbita_G.c -o ../bin/queorbita_G -lm 
...
main()
{
  float	
    ...
    gx,                        /* coordenada X */
    gy,                        /* coordenada Y */
    gz,                        /* coordenada Z (índice do loop) */
    x,                         /* coordenada X (dado de entrada) */
    y,                         /* coordenada Y (dado de entrada) */
    z,                         /* coordenada Z  */
    vx,                        /* velocidade X */
    vy,                        /* velocidade Y */
    thetav,
  ...
  /* Escolha dos gz */
  gzini=-2.*sqrt(gx*gx + gy*gy); 
/*   gzini=-2.*sqrt(gx*gx + gy*gy) * 0.2 ;  */
  gzfin=-gzini; 
  gzstep=(gzfin - gzini)/100.;  
  ...
  puts("	-----------------------------------------");
  puts("	");
  puts("e	q	rnow	vq	gx	gy	gz	VX_ceu	VY_ceu	VZ_ceu	x	y	vx	vy	spx	spy	spz	qx	qy	qz	vsys	Dir	spin-orb	PERIC	pos-peri");
  

  for (e=eini;e<=efin;e=e+estep) {
    for (q=qini;q<=qfin;q=q+qstep) {
      if (e < 1.0) {
	/* eliptica */
	orb=(int)0;
	a=q/(1.-e);
	...
	vq=sqrt(2*mu*( 1/q  + 1/(2*a) ));
	 ...
	 sobre o plano XY (atrás). */
      ...
      for (gz=gzini;gz<=gzfin;gz=gz+gzstep){
	...
	case 0:
	  ...
	  if (rnow < apocenter) {
	    ...
	    if (e == 0.0) {
	      ...
	      aux=(h*h/(rnow*mu) - 1)/e ;  
	      aux=aux-0.000001;         /* Para evitar problemas 
	...
	*/
	...
	if ( (fabs(v) >= fabs(vsys)) && ((orb == 0 && rnow <= apocenter) || (orb > 0)) ) {
	  ...
	  for (pre_pos_peri=1 ; pre_pos_peri <= 2 ; pre_pos_peri=pre_pos_peri+1){ 
	  ...
	  for (i=0;i<=355;i=i+5) {
	    ...
	    if (vv[2] >= vsys-vsyser && vv[2] <= vsys+vsyser) {  
